...
class WB_MagicPost_Baidu_Api
{
    ...
    public function get_token()
    {
        ...
        if(is_wp_error($http)){
            $this->txt_log('BaiduAPI获取token失败。错误信息:');
            $this->txt_log($http->get_error_message());
            return false;
        }
        $body = wp_remote_retrieve_body($http);
        if (empty($body)) {
            $this->txt_log('BaiduAPI获取token失败。响应为空');
            return false;
        }
        $token = json_decode($body,true);

        if(isset($token['error'])){
            $this->txt_log('BaiduAPI获取token失败。Err:'.$token['error'].','.$token['error_description']);
            return false;
        }
    ...
    public function doc_translate_result($doc_id)
    {
        ...
        $http = wp_remote_post($api, $param);
        if (is_wp_error($http)) {
            $this->txt_log('百度文档翻译查询结果失败。错误信息:');
            $this->txt_log($http->get_error_message());
            return false;
        }
        $body = wp_remote_retrieve_body($http);
        ...
        //
        if(empty($result)){
            $this->txt_log('百度文档翻译查询失败。响应解析错误');
            return false;
        }
        if(isset($result['error_code'])){
            $this->txt_log('百度文档翻译查询错误,'.$result['error_code'].','.$result['error_msg']);
            return false;
        }
    ...
    public function doc_translate($filename, $text)
    {
        ...
        $http = wp_remote_post($api, $param);
        if (is_wp_error($http)) {
            $this->txt_log('百度文档翻译提交文档失败。错误信息:');
            $this->txt_log($http->get_error_message());
            return false;
        }
        $body = wp_remote_retrieve_body($http);
        ...
        //
        if(empty($result)){
            $this->txt_log('百度文档翻译失败。响应解析错误');
            return false;
        }
        if(isset($result['error_code'])){
            $this->txt_log('百度文档翻译错误,'.$result['error_code'].','.$result['error_msg']);
            return false;
        }
