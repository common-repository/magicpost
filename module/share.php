...
class WB_MagicPost_Share extends WB_MagicPost_Base
{
	...
	function dwqr_html($attr = array())
	{
		...
		$selected_module = !empty($attr) && isset($attr['selected_module']) ? $attr['selected_module'] : self::opt('dwqr_module');
		$def_wpclass = wp_is_mobile() || !$selected_module['position'] ? 'wbp-cbm-magicpost' : 'wbp-dwqr-sticky left';
		$wp_classname = !empty($attr) && isset($attr['wpclass']) ? $attr['wpclass'] : $def_wpclass;
	...
	function get_share_html()
	{
		...
		foreach ($share_items as $k => $v) {
			$wb_dwqr_share_html .= '<a class="share-logo-magicpost icon-' . $k . '" data-cmd="' . $k . '" title="分享到' . $v['name'] . '" rel="nofollow"><svg class="wb-icon wbsico-magicpost-' . $k . '"><use xlink:href="#wbsico-magicpost-' . $k . '"></use></svg></a>';
		}
