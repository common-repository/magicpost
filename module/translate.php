...
class WB_MagicPost_Translate extends WB_MagicPost_Base
{
    ...
    public function restrict_manage_posts($post_type, $which)
    {
        if (current_user_can('edit_posts') && get_option('wb_magicpost_ver', 0)) {
            ...
            echo '<select name="is_trans"><option value="">翻译状态</option>';
            foreach (array(1 => '已翻译', 2 => '未翻译') as $k => $v) {
        ...
        if ($this->can_use() && current_user_can('edit_post', $post->ID)) {
            ...
            do {
                ...
                if($api == 'baidu'){
                    ...
                    if($doc_id){
                        $actions['magicpos_translate_doc_query_action'] = '<a class="magicpos_translate_doc_query_action" data-post_id="' . $post->ID . '" href="javascript:;">查询翻译</a>';
                    }
                }
                ...
                $actions['magicpos_translate_action'] = '<a class="magicpos_translate_action" data-post_id="' . $post->ID . '" href="javascript:;">翻译</a>';
            } while (0);
    ...
    public function bulk_actions($actions)
    {
        ...
        if ($this->can_use() && current_user_can('administrator')) {
            $actions['magicpost_translate'] = '批量翻译';
    ...
    public function translate_single_post($post_id)
    {
        ...
        if ($ret['code']) {
            ...
            $err = '文章：' . $post->post_title . ' 翻译失败，错误：' . $ret['desc'];
    ...
    public function magic_post_baidu_translate_get_doc($post_id)
    {
        ...
        do{
            if($ret === false){
                $error = '文章：' . $post->post_title . ' 百度获取文档翻译失败，错误：' . $obj->get_error();
                break;
            }
            $result = $ret['result'];
            if(!isset($result['data']) || empty($result['data'])){
                $error = '文章：' . $post->post_title . ' 百度获取文档翻译失败，data 为空';
                break;
            }
            $data = $result['data'];
            if($data['status'] == 'Failed'){
                $error = '文章：' . $post->post_title . ' 百度获取文档翻译失败，'.$data['reason'];
                break;
            }
            if($data['status'] == 'Succeeded'){
                ...
                foreach($doc_list as $doc){
                    ...
                    if(is_wp_error($http)){
                        ...
                        $error = '获取翻译结果出错，'.$http->get_error_message();
                    } else {
            ...
            $message = ['NotStarted'=>'待翻译', 'Running'=>'翻译中'][$data['status']]  ?? '翻译失败';
    ...
    public function magicpost_ajax()
    {
        ...
        $arrow = [
            'baidu_doc_query', 'translate_row', 'translate_setting', 'translate_update'
        ];
