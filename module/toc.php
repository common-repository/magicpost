...
class WB_MagicPost_Toc extends WB_MagicPost_Base
{
  ...
  public static $cnf_toc = array(
    'ct_mode' => array('2' => 'H2', '3' => 'H3', '23' => 'H2 & H3',  '34' => 'H3 & H4')
  );
  ...
  {
    $is_pro = get_option('wb_magicpost_ver', 0);
  ...
  {
    wp_register_sidebar_widget('magicpost-toc-widget', '#Magicpost 内容目录#', array($this, 'widget_handler'), array('description' => '侧栏展示内容目录'));
  }
  ...
  public function widget_handler()
  {
    ...
    echo $this->toc_html($args);
  }

  function toc_shortcode_handler()
  {
    return $this->toc_html(['location' => 'content', 'class' => 'toc-on-content']);
  }
  ...
  public function the_content_handler($content)
  {
    ...
    if (is_single() && self::opt('auto_insert_content') == 1) {
      $content = $this->toc_html() . $content;
    }
   ...
   * @return string
   */
  function toc_html($args = ['location' => 'content', 'class' => 'toc-on-content'])
  {
    $post_id = get_the_ID();
    ...
    if ($tc_mode) {
      ...
      foreach ($matches[1] as $index => $tag) {
        ...
        if ($location == 'content') {
          if ($cnf_tags[0] == $tag) {
            $sc_items[$index] = '<a class="mgp-toc-title" href="#toc-' . $idx . '">' . $item . '</a>';
            $idx++;
          } elseif (isset($cnf_tags[1]) && $cnf_tags[1] == $tag) {
            $sc_items[$index] = '<a class="mgp-toc-subtitle" href="#toc-s-' . $idxs . '">' . $item . '</a>';
            $idxs++;
          }
        } else {
          if ($cnf_tags[0] == $tag) {
    ...
    if (count($sc_items) > 1) {
      ...
      $toc_label = self::opt('toc_label') ? self::opt('toc_label') : '内容目录';
      $title_html =  '';
      $style =  '';

      if ($location == 'widget') {
        $style .= self::opt('style_widget_max') ? '--mgp-toc-mxh: ' . (int)self::opt('style_widget_max') * 2 . 'em;' : '';
        $style .= $this->style_output('widget');
        ...
        $style .= $this->style_output('content');
   ...
   * @return string
   */
  function style_output($location)
  {
    if (!$location) return;
