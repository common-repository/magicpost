...
class WB_MagicPost_Schedule extends WB_MagicPost_Base
{
    ...
    public function bulk_actions($actions)
    {
        ...
        if (current_user_can('administrator')) {
            ...
            $actions['magicpost_batch'] = '加入定时发布';
    ...
    public function magicpost_ajax()
    {
        ...
        $arrow = [
          'schedule_post', 'schedule_setting', 'schedule_update', 'schedule_batch'
        ];
        ...
        switch ($op) {
            ...
            case 'schedule_setting':
                ...
                $ret['cnf'] = [
                    'post_type' => $post_types ? $post_types : ['post' => '文章', 'page' => '页面'],
                    'post_status' => ['draft' => '草稿', 'pending' => '待审'],
                    'source' => ['auto' => '自动添加至定时发布', 'manual' => '手动添加至定时发布'],
                    'week' => [
                        ['w' => '周一', 'v' => 1, 'day' => ''],
                        ['w' => '周二', 'v' => 2, 'day' => ''],
                        ['w' => '周三', 'v' => 3, 'day' => ''],
            ...
            case 'schedule_batch':
                ...
                do {
                    ...
                    if ($state) {
                        ...
                        $ret['desc'] = '提交成功';
                        if ($num > $state) {
                            $ret['desc'] = '成功定时处理' . $state . '篇文章。其中' . ($num - $state) . '篇不符合定时设置';
                        }
                    } else {
                        $ret['desc'] = '没有需要定时处理的文章。';
                    }
                } while (0);
                self::ajax_resp($ret);
                break;
